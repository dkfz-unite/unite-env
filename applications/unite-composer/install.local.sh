...
GITHUB_USER=$ghb_usr GITHUB_TOKEN=$ghb_tkn ELASTIC_USER=$els_usr ELASTIC_PASSWORD=$els_pwd MYSQL_USER=$sql_usr MYSQL_PASSWORD=$sql_pwd docker-compose -p '' -f docker-compose.local.yml up -d --build